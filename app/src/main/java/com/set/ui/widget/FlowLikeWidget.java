...
public class FlowLikeWidget extends RelativeLayout {
    ...
    public FlowLikeWidget(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public FlowLikeWidget(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void addLikeView() {
        ...
        likeView.setImageDrawable(mLikeDrawables.get(mRandom.nextInt(mLikeDrawables.size())));//随机找到Drawable
        likeView.setLayoutParams(mLayoutParams);

        addView(likeView);
        startAnimation(likeView);//开始动画
    }
    ...
    private ValueAnimator generateCurveAnimation(View target) {
        CurveEvaluator evaluator = new CurveEvaluator(generateCTRLPointF(1), generateCTRLPointF(2));//三阶贝塞尔曲线的2个控制点
        ValueAnimator valueAnimator = ValueAnimator.ofObject(evaluator,
                new PointF((mViewWidth - mPicWidth) / 2, mViewHeight - mChildViewHeight - mPicHeight),//开始位置
                new PointF((mViewWidth) / 2 + (mRandom.nextBoolean() ? 1 : -1) * mRandom.nextInt(100), 0));//结束位置
